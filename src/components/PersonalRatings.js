  ...
  const fetchRatings = async () => {
    ...
    if(urlParams.rating !== 'all') {
      sortedData.forEach(el => {
        if (el.rating === urlParams.rating) {
          targetRating = [...targetRating, el];
        }
      })
    } else {
      targetRating = sortedData;
    }
    setUserRatings(targetRating);
    setDisplayedRatings(targetRating.slice(0, 25))
  }
